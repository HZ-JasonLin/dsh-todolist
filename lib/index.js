 ...
 *
 * 完全自建的待办系统，使用独立的本地存储：
 * - 存储：<dataDir>/todos.json（全部待办）、<dataDir>/suggestions.json（AI 提议待确认队列）
 * - API：/todolist/api/todo（list/add/done/update/remove）、/todolist/api/suggestions（list/approve/reject）
 * - AI 工具：todolist（操作待办）、todolist_suggest（审查提议待办）
 * - 快照：注入使用说明 + 每轮收尾到期检查 + 审查提议提示
 *
...
export const name = 'dsh-todolist'
export const inject = ['tools', 'systemPrompt', 'webServer']

const DEFAULTS = {
  dataDir: null,             // null → <dshHome>/todo-data
  toolName: 'todolist',
  suggestToolName: 'todolist_suggest',
  snapshotOrder: 420,
  maxListView: 8,
}
...
export function resolveConfig(raw) {
  ...
  if (typeof config.suggestToolName !== 'string' || !config.suggestToolName) throw new Error('dsh-todolist: suggestToolName 必须是非空字符串')
  return config
}
...
export class SuggestionStore {
  ...
  all() {
    return new JsonStore(this.file, []).load()
  }
  save(items) {
  ...
  add(entry) {
    const items = this.all()
    items.push({ ...entry, id: newId(), createdAt: nowStamp() })
    this.save(items)
  }
  removeById(id) {
    const items = this.all()
...
export function installApi(ctx, deps) {
  const { store, suggestStore, config } = deps
  const handler = async (req, res) => {
    try {
      ...
      if (method === 'GET' && path === '/todolist/api/suggestions') {
        sendJson(res, 200, { entries: suggestStore.all() })
        return
      }

      if (method === 'POST' && path === '/todolist/api/suggestions') {
        const body = await readBody(req)
        const action = String(body?.action ?? '')
        if (action === 'approveAll') {
          // 后端一次性采纳整个队列：消除前端逐个按 index 操作的下标位移竞态
          const entries = suggestStore.all()
          if (entries.length === 0) { sendJson(res, 400, { ok: false, message: '没有待处理的建议' }); return }
          const failed = []
          let okCount = 0
          for (const entry of entries) {
            const outcome = store.add({
              content: entry.content,
              proj: entry.proj, who: entry.who,
              start: entry.start, due: entry.due, repeat: entry.repeat, on: entry.on,
              quadrant: entry.quadrant,
            })
            if (outcome.ok) okCount += 1
            else failed.push(entry)
          }
          // 采纳失败的字段不合法建议放回队列，不丢数据
          suggestStore.save(failed)
          const msg = `${okCount > 0 ? `已采纳 ${okCount} 条并写入待办` : '没有可采纳的建议'}${failed.length > 0 ? `，${failed.length} 条字段不合法已保留在队列` : ''}`
          sendJson(res, okCount > 0 ? 200 : 400, { ok: okCount > 0, message: msg })
          return
        }
        const id = String(body?.id ?? '')
        if (!id) { sendJson(res, 400, { ok: false, message: 'id 不能为空' }); return }
        if (action === 'approve') {
          const entry = suggestStore.removeById(id)
          if (!entry) { sendJson(res, 400, { ok: false, message: '建议不存在或已被处理' }); return }
          const outcome = store.add({
            content: entry.content,
            proj: entry.proj, who: entry.who,
            start: entry.start, due: entry.due, repeat: entry.repeat, on: entry.on,
          ...
          if (!outcome.ok) {
            // 字段不合法 → 放回队列，不丢建议
            suggestStore.add(entry)
            sendJson(res, 400, { ok: false, message: `采纳失败（建议字段不合法）：${outcome.message}` })
            return
          }
          sendJson(res, 200, { ok: true, message: '已采纳并写入待办' })
          return
        }
        if (action === 'reject') {
          const entry = suggestStore.removeById(id)
          if (!entry) { sendJson(res, 400, { ok: false, message: '建议不存在或已被处理' }); return }
          sendJson(res, 200, { ok: true, message: '已拒绝' })
          return
        }
        sendJson(res, 400, { ok: false, message: `未知操作 "${action}"` })
        return
...
export function todoToolDefinition(config, store) {
  return {
    ...
    description: '待办管理（独立待办看板）。用户口述"记住/我要做 X"时用 add 直写（用户即确认者）；AI 自建待办请用 todolist_suggest（进待确认队列），不要直接 add。**list 默认智能视图**：只返回需要关注的未完成项（逾期/今日到期/今日周期/重要紧急，最多 8 条），看全部需显式 all=true 或筛选参数。每条待办支持：proj=项目（项目名称由任务数据决定）、who=负责人、due=截止日期、start=开始日期（与 due 组成时间跨度）、repeat=周期（daily/weekly/monthly，可配 on 定点：weekly 填 1-7=周几、monthly 填 1-31=几号；周期任务完成后自动推进到下一截止）、quadrant=四象限（q1-q4）、status=状态。done/update/remove 按 id 精确操作（list 输出带 id）。',
    parameters: {
      type: 'object',
      properties: {
...
/** `todolist_suggest` 工具：AI 审查提议待办（进待确认队列，用户确认后生效）。 */
export function suggestToolDefinition(config, suggestStore) {
  return {
    name: config.suggestToolName,
    description: '向待确认队列提议一条待办（AI 自建待办专用）：审查/对话中发现值得记录的任务时使用，用户确认后写入待办；不要用 todolist add 直接添加 AI 自建的待办。',
    parameters: {
      type: 'object',
      properties: {
        content: { type: 'string', description: '待办内容（必填，首行是标题）' },
        proj: { type: 'string', description: '项目归属（如 设计/开发/运营）' },
        who: { type: 'string', description: '负责人' },
        due: { type: 'string', description: '截止日期 YYYY-MM-DD' },
        start: { type: 'string', description: '开始日期 YYYY-MM-DD' },
        repeat: { type: 'string', enum: REPEATS, description: '周期循环' },
        on: { type: 'string', description: '周期定点：weekly 1-7=周几、monthly 1-31=几号' },
        quadrant: { type: 'string', enum: ['q1', 'q2', 'q3', 'q4'], description: '四象限' },
      },
      required: ['content'],
    },
    output: {
    ...
    async execute(args) {
      ...
      suggestStore.add({
        content,
        proj: typeof args.proj === 'string' && args.proj.trim() !== '' ? args.proj.trim() : null,
        who: typeof args.who === 'string' && args.who.trim() !== '' ? args.who.trim() : null,
        start: typeof args.start === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(args.start) ? args.start : null,
        due: typeof args.due === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(args.due) ? args.due : null,
        repeat: REPEATS.includes(args.repeat) ? args.repeat : null,
        on: typeof args.on === 'string' && /^\d{1,2}$/.test(args.on) ? Number(args.on) : null,
        quadrant: /^q[1-4]$/.test(args.quadrant ?? '') ? args.quadrant : null,
      })
      return { ok: true, message: '已提议待办（等待用户确认）' }
    },
  }
}
...
export function renderUsage(config) {
  return [
    `## 待办（dsh-todolist 插件）`,
    `- 管理：用户口述"记住/我要做 X"用 ${config.toolName} add 直写（用户即确认者）；AI 自建待办用 ${config.suggestToolName}（进待确认队列，用户确认后生效）。`,
...
export function apply(ctx, rawConfig = {}) {
  ...
  const suggestStore = new SuggestionStore(join(config.dataDir, 'suggestions.json'))

  // AI 工具
  ctx.effect(() => ctx.tools.register(todoToolDefinition(config, store)), 'dsh-todolist: todo tool')
  ctx.effect(() => ctx.tools.register(suggestToolDefinition(config, suggestStore)), 'dsh-todolist: suggest tool')
  ...
  ctx.inject(['webServer'], (webCtx) => {
    webCtx.effect(() => installApi(webCtx, { store, suggestStore, config }), 'dsh-todolist: web api')
  })
}
